...
func buildArgv(tmpl []string, f flags, group []string) []string {
	if f.replace != "" {
		return substitute(tmpl, f.replace, inputLine(strings.Join(group, " ")))
	}
...
// inputLine is one -I input line: the text substituted for every occurrence of
// the replace-token in the command template.
type inputLine string

// substitute replaces every occurrence of the -I token in each template
// element with the input line (the -I behavior).
func substitute(tmpl []string, token XargsReplace, line inputLine) []string {
	out := make([]string, len(tmpl))
	for i, t := range tmpl {
		out[i] = strings.ReplaceAll(t, string(token), string(line))
	}
	return out
}
...
// add appends fields, emitting a group whenever the -n size is reached, and
// returns the updated grouper.
func (g grouper) add(fields []string) grouper {
...
// emit sends the buffered group and returns the grouper with a fresh buffer.
func (g grouper) emit() grouper {
