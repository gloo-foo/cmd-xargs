...
Compatible with GNU `xargs` for the implemented flags. With a command, output is byte-identical to GNU `xargs` for the default exec, `-n`, `-I`, `--null`, and `-P` behaviors (verified in the `yupsh/bin-xargs` Docker integration harness against Debian findutils, using `echo` as the target). The one divergence is the short `-0` spelling.
